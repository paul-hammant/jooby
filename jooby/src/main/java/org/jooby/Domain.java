...
public class Domain implements Joobyable {
   ...
   * @param joobyable The Domain is mounted inside that Jooby or Dir or Domain.
   */
  public Domain(final String domainName, Joobyable joobyable) {
    this.joobyable = joobyable;
    representativeName = "[domain=" + domainName + "]";
    use("*", (req, rsp, chain) -> {
      if (req.hostname().equals(domainName)) {
        chain.next("/" + representativeName, req, rsp);
      } else {
        chain.next(req, rsp);
      }
    });
  ...
  public Definition get(final String path, final Route.OneArgHandler handler) {
    ...
    if (joobyable instanceof Jooby) {
      return ((Jooby) joobyable).get(path1, handler).name(representativeName);
    } else if (joobyable instanceof Dir) {
      return ((Dir) joobyable).get(path1, handler).name(representativeName);
    } else {
      throw new UnsupportedOperationException("domain in domain not supported");
    }
