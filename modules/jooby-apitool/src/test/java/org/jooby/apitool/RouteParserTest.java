  ...
  public void apiLike() throws Throwable {
    new RouteMethodAssert(new ApiParser(dir()).parse(new AppWithDoc()))
        ...
        .next(r -> {
          ...
          r.returnType(LocalType.class);
          r.summary("Api summary.");
          r.description("Get a pet by ID.");
          r.returns(
              "A {@link LocalType} with a <code>200 = Success</code> code or <code>404</code>.");
          r.param(p -> {
            p.name("id")
        ...
        .next(r -> {
          r.method("GET");
          r.pattern("/api/pets");
          r.returnType(listOf(LocalType.class));
          r.param(p -> {
            p.name("start")
                .type(int.class)
                .value(0);
